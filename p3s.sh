...
sleep 3
    			  ~/bin/prepscan.sh -x $USEX  -y $USEY  -r 150 -v "$DEVICE" -d $SDIR -f Rs$i
          else
					/usr/bin/scanimage -x $x  -y $y --format=tiff --resolution $RES --mode $MYMODE --device $DEVICE --source $SCANSOURCE>$SDIR/s$i.tif
sleep 1
					/usr/bin/scanimage -x $x  -y $y --format=tiff --resolution 150 --mode $MYMODE --device $DEVICE --source $SCANSOURCE>rjpg/Rs${i}.tif
          fi
				break
			else
				if [ $savex ]; then
