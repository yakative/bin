...
#
# also added in this version is a prompt for resolution.  As soon as you
# set a resolution from the prompt, it remains in effect.  If you skip over
# resolution setting, whatever is specified on the command line will be used,
# and if nothing is entered on command line, defaults to 100dpi
...
mysql -u "$DBUSER" -p"$DBPASS" "$DBNAME" <<EOF
	INSERT into ya_cat values ("$CUSTCODE","$i","$savebox");
EOF
